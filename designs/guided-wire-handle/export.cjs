...
let views={assembly:m.main({view:'assembly',travel:0}),handle:m.main({view:'handle'}),layout:m.main({view:'layout'}),cover:[m.main({view:'cover'})],uncovered:m.main({view:'cutaway'})};
...
console.log('Exported source sections and '+Object.keys(views).length+' CAD views.');
